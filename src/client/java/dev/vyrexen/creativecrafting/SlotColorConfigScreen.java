...
public class SlotColorConfigScreen extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            int sy = startY;
            context.fill(sx,                sy,                sx + slotSize,     sy + 1,             shadow);
            context.fill(sx,                sy + 1,            sx + 1,            sy + slotSize - 1,  shadow);
            context.fill(sx + slotSize - 1, sy + 1,            sx + slotSize,     sy + slotSize - 1,  hilight);
            context.fill(sx,                sy + slotSize - 1, sx + slotSize,     sy + slotSize,      hilight);
            context.fill(sx + 1,            sy + 1,            sx + slotSize - 1, sy + slotSize - 1,  fill);
        }
